...
class Strategy (TraderStrategy):
    ...
    def run_strategy(self):
        ...
        ops["a_atr"] = hkdtl.ATR(20,strip=False)
        ops["b_std"] = hkdtl.STDEV(hkdtl.close,20,strip=False)
        #ops["pp_kama"] = hkdtl.KAMA(10,2,30,strip=True)
        #ops["pp_kama"] = hkdtl.KAMA(34,15,34,strip=True)
        
        ops["zzbuyfast"],ops["zzperbuyfast"],ops["zzimperbuyfast"],obsLow,obsHigh = hkdtl.ZIGZAG(hkdtl.start_index,hkdtl.max_view,2,6,close=False)
        ops["zzbuyslow"],ops["zzperbuyslow"],ops["zzimperbuyslow"],obsLow,obsHigh = hkdtl.ZIGZAG(hkdtl.start_index,hkdtl.max_view,4,12,close=False) #hkdtl.ZIGZAG(hkdtl.start_index,hkdtl.max_view,2,6)

        #ops["zzbuyfast"],ops["zzperbuyfast"],ops["zzimperbuyfast"],obsLow,obsHigh = hkdtl.AZIGZAG(hkdtl.start_index,hkdtl.max_view,1,3,1.1,20,6)
        #ops["zzbuyfast"],ops["zzperbuyfast"],ops["zzimperbuyfast"],obsLow,obsHigh = hkdtl.AZIGZAG(hkdtl.start_index,hkdtl.max_view,1,3,1.1,20,6)
        #ops["zzbuyslow"],ops["zzperbuyslow"],ops["zzimperbuyslow"],obsLow,obsHigh = hkdtl.AZIGZAG(hkdtl.start_index,hkdtl.max_view,1,3,1.1,20,6)        
    ...
    def buy(self,ops,index):
        ...
        if(ops["zzimperbuyfast"][index][1]==1):
            return True
    ...
    def sell(self,ops,index):
        ...
        diff_add = diff_1+diff_2+diff_3+diff_4+diff_5+diff_6
        '''
        if(ops["zzimperbuyslow"][index][1]==0 or ops["zzimperbuyfast"][index][1]==0):
            return True
        
        '''
        
        if(ops["zzimperbuyfast"][index][1]==0):
            return True
