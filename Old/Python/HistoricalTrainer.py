...
if (add_indicator==True):
    ...
    srsi = tim.calculateSRSI_Of_Index(rsi,9)
    k,d = tim.calculateSTO(srsi,3,3)
    ...
    pricePred = model.predict(np.array(xTrain[len(xTrain)-100:len(xTrain)]))
    priceCompile = []
    ...
    price = np.array(yTrain[len(yTrain)-100:len(yTrain)])
    reaPriceCompile = []
    ...
    #.plot(yTrain[len(xTrain)-100:len(xTrain)], color = "red")
    #plt.show()
    ...
    for i in range(0,int(epochs/saveEpoch)):
        ...
        plt.plot(yTrain[len(xTrain)-100:len(xTrain)], color = "red")
        plt.show()
        '''
